...
export default function ReportView({ activeReport, setView }) {
  ...
  const exportToDoc = () => {
    let groupsHtml = '';
    const cover = `<table width="450" align="center" style="border: 6pt double #1a365d; background-color: #f0f7ff; margin-bottom: 30pt;" cellpadding="40"><tr><td align="center"><div style="font-size: 14pt; color: #3498db; font-family: Arial;">DIPLOMAT GROUP</div><h1 style="font-size: 32pt; color: #1a365d; font-family: Arial;">${activeReport.title}</h1><div style="width: 100pt; border-top: 3pt solid #3498db; margin: 15pt auto;"></div><p style="font-size: 18pt; color: #2c3e50; font-weight: bold; font-family: Arial;">דוח סיור חנות מקצועי</p><p style="font-size: 12pt; color: #7f8c8d; font-family: Arial;">תאריך: ${new Date(activeReport.createdAt).toLocaleDateString('he-IL')}</p></td></tr></table><br style="page-break-before:always; mso-break-type:section-break;" />`;

    activeReport.groups.forEach((g) => {
      const items = g.items || [...(g.images?.map(u => ({ type: 'image', url: u })) || []), ...(g.notes?.map(t => ({ type: 'note', text: t })) || [])];
      const notes = items.filter(i => i.type === 'note');
      const images = items.filter(i => i.type === 'image');

      let gHtml = `<table width="450" align="center" style="border: 1.5pt solid #1a365d; table-layout: fixed;" cellpadding="15"><tr><td align="right">`;
      gHtml += `<h2 style="font-size: 18pt; color: #1a365d; border-bottom: 1pt solid #eee; font-family: Arial; margin: 0 0 10pt 0;">${g.title || 'קבוצה'}</h2>`;
      notes.forEach(n => { gHtml += `<div style="padding: 8pt; border-right: 4pt solid #3498db; background: #f4f7f9; margin-bottom: 6pt; font-family: Arial; font-size: 12pt; direction: rtl;">${n.text}</div>`; });

      if (images.length > 0) {
        const cols = images.length === 1 ? 1 : (images.length <= 4 ? 2 : 3);
        const imgW = Math.floor(400 / cols);
        gHtml += `<table width="100%" cellspacing="5"><tr>`;
        images.forEach((img, idx) => {
          if (idx > 0 && idx % cols === 0) gHtml += `</tr><tr>`;
          gHtml += `<td align="center" valign="top" style="border: 0.5pt solid #eee; padding: 5pt;"><img src="${img.url || img.localUrl}" width="${imgW}" style="width: ${imgW}pt; height: auto;" />${img.note ? `<div style="background: #fdf9e7; padding: 3pt; font-size: 9pt; font-family: Arial; margin-top: 4pt; direction: rtl; border-right: 2pt solid #f1c40f;">${img.note}</div>` : ''}</td>`;
        });
        const rem = (cols - (images.length % cols)) % cols;
        for(let i=0; i<rem; i++) gHtml += `<td></td>`;
        gHtml += `</tr></table>`;
      }
      gHtml += `</td></tr></table><br style="page-break-before:always; mso-break-type:section-break;" />`;
      groupsHtml += gHtml;
    });

    const body = `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset='utf-8'><style>@page { size: 595pt 842pt; margin: 40pt; } body { font-family: Arial; direction: rtl; }</style></head><body>${cover}${groupsHtml}</body></html>`;
...
const imageWrapperStyle = { position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fdfdfd', overflow: 'hidden' };
